...
X=X.drop(X.index[indicesToDelete]).reset_index(drop=True)
y=y.drop(y.index[indicesToDelete]).reset_index(drop=True)
#print(X)
#exit()
...
logreg = GridSearchCV(LogisticRegression(), possible_parameters, n_jobs=4, cv=3) # n_jobs=4 means we parallelize the search over 4 threads
